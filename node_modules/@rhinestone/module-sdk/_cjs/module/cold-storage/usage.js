...
const getColdStorageSetWaitPeriodAction = ({ waitPeriod, }) => {
    ...
    return {
        to: constants_1.COLD_STORAGE_HOOK_ADDRESS,
        target: constants_1.COLD_STORAGE_HOOK_ADDRESS,
        value: BigInt(0),
        callData: data,
        data,
    };
...
const getColdStorageExecutionTime = async ({ account, client, executionHash, }) => {
    try {
        const executionTimestamp = (await client.readContract({
            address: constants_1.COLD_STORAGE_HOOK_ADDRESS,
            abi: abi_1.abi,
...
const getFlashloanAddAddressAction = ({ addressToAdd, }) => {
    ...
    return {
        to: constants_1.COLD_STORAGE_FLASHLOAN_ADDRESS,
        target: constants_1.COLD_STORAGE_FLASHLOAN_ADDRESS,
        value: BigInt(0),
        callData: data,
        data,
    };
...
const getFlashloanRemoveAddressAction = async ({ client, account, addressToRemove, }) => {
    ...
    if (currentAddressIndex === -1) {
        ...
        prevAddress = constants_2.SENTINEL_ADDRESS;
    }
    else {
...
const getFlashloanWhitelist = async ({ account, client, }) => {
    try {
        const whitelistAddresses = (await client.readContract({
            address: constants_1.COLD_STORAGE_FLASHLOAN_ADDRESS,
            abi: flashloanAbi_1.abi,
